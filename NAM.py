...
class NAMTuringRecurrent(nn.Module):
    ...
    def forward_step(self, next_tgt, tape, keytape, rpos, wpos, cntl_hidden):
        bsize = next_tgt.shape[0]
        prev_rval = torch.einsum('lntc,lnt->ntc',tape, rpos).reshape(bsize,-1)
        h,c = self.controller(torch.concat((next_tgt,prev_rval),dim=-1), cntl_hidden)
        #h,c = self.controller(next_tgt, cntl_hidden)
        qkva = self.Wqkva(h)
        query = qkva[:,:self.head_dim*self.n_tapes].reshape(bsize, self.n_tapes, -1)
        query = F.normalize(query,dim=-1)
        key = qkva[:,self.head_dim*self.n_tapes:2*self.head_dim*self.n_tapes].reshape(bsize, self.n_tapes, -1)
        ...
        oldkey = torch.einsum('lntc,lnt->ntc',keytape, wpos)
        newkey = torch.einsum('lnt,ntc->lntc',wpos,(key*rwe[:,:,1:2]-oldkey*rwe[:,:,2:3]))
        nkeytape = keytape + newkey
        ...
        return F.relu(self.outlayer(rval.reshape(bsize,-1))+h), ntape, nkeytape, nrpos, nwpos, (h,c)
        
    def init_states(self, src, tapelen_in=-1):
        batchsize = src.shape[1]


        tapelen_in = self.default_tapelen if tapelen_in <= 0 else tapelen_in
        tape = torch.zeros([tapelen_in, batchsize, self.n_tapes, self.head_dim],
            dtype=src.dtype, device=src.device)
        keytape = torch.zeros_like(tape)
        ...
        return tape, keytape, rpos, wpos, cntl_hidden
    #Seq-first in (S,N,C), seq-first out (S,N,C)
    #Stack: initial stack state (zeros or null embeddings)
    def forward(self, inputs, tapelen=-1):
        ...
        tape, keytape, rpos, wpos, cntl_hidden = \
            self.init_states(inputs, tapelen)

        read_outs=[]
        for i in range(seqlen):
            rval, tape, keytape, rpos, wpos, cntl_hidden = \
                self.forward_step(inputs[i], tape, keytape, rpos, wpos, cntl_hidden)
